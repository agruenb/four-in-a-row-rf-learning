...
#added for testing a negative reward for loosing games
def mcts_player_neg_reward(env,chip_,rollouts):
...
def back_propagate(tree, node_id, chip_):
    ...
    while tree[node_id]['parent'] is not None:
        ...
        if tree[node_id]['player'] == chip_:
            reward = 1
        elif tree[node_id]['player'] == get_opposite_player(chip_):
            reward = -1
        else:
            reward = 0
